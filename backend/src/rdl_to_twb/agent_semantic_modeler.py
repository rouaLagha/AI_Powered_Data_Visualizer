...
def generate_semantic_models(
    ...
    if write_artifacts:
        ...
        # Only write the three required abstract-model JSON files.
        (output_dir / "data_model.json").write_text(
            json.dumps(data_model, indent=2, ensure_ascii=True), encoding="utf-8"
        )
        # The pipeline's intermediate "visual model" is exported as the
        # canonical `visual_elements.json` for downstream Power BI generation.
        (output_dir / "visual_elements.json").write_text(
