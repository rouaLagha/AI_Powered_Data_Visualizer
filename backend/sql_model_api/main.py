...
from src.qlik_to_powerbi.metadata_pipeline import (
    generate_powerbi_report_from_metadata_job,
...
def _run_qlik_powerbi_report_generation_endpoint(payload: dict[str, Any]) -> dict[str, Any]:
    jobs_root_value = str(payload.get("jobs_root") or payload.get("output_dir") or "").strip()
    jobs_root = _resolve_workspace_path(jobs_root_value) if jobs_root_value else QLIK_POWERBI_JOBS_DIR
    job_id = str(payload.get("job_id") or "").strip()
    job_dir_value = str(payload.get("job_dir") or "").strip()
    if job_dir_value:
        job_dir = _resolve_workspace_path(job_dir_value)
    elif job_id:
        job_dir = jobs_root / job_id
    else:
        raise ValueError("Provide job_id or job_dir before generating the Power BI report.")

    try:
        result = generate_powerbi_report_from_metadata_job(
            job_dir=job_dir,
            pbip_template_path=str(
                payload.get("pbip_template_path")
                or payload.get("powerbi_template_path")
        ...
        fallback_job_id = job_id or job_dir.name or _new_qlik_job_id()
        result = _write_failed_qlik_job(jobs_root=jobs_root, job_id=fallback_job_id, exc=exc)
...
POST_HANDLERS = {
    ...
    "/api/qlik-powerbi/report/generate": _run_qlik_powerbi_report_generation_endpoint,
