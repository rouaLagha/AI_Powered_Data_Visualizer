...
- Utilise le schema `backend/assets/ReportDefinition.xsd` pour contraindre les types visuels extraits.
...
- Un script best effort est disponible: `backend/scripts/publish_tableau_public_desktop_rpa.py`.
...
```bash
python backend/scripts/publish_tableau_public_desktop_rpa.py --twbx outputs/converted_report.twbx
```
...
La barre de navigation React expose maintenant:
- `SQL model flow`: pipeline interactif existant.
- `RDL to TWB`: conversion directe multi-agent RDL vers TWB.
- `RDL AI editor`: edition d un RDL par instruction, avec sortie RDL modifiee telechargeable.
...
```bash
python backend/run_conversion.py --rdl path/to/report.rdl --config backend/config/llm_config.example.json
```

Mode conversion-only (sans extract/publish/RPA):

```bash
python backend/run_conversion.py --rdl path/to/report.rdl --config backend/config/llm_config.json --no-publish
```

Si `--config` n est pas fourni, le CLI prend automatiquement:
- `backend/config/llm_config.json` si le fichier existe
- sinon `backend/config/llm_config.example.json`
...
- sortie de diagnostic: `outputs/validation_report.json`
...
Vous pouvez configurer `agent2` vers un endpoint local compatible OpenAI Chat Completions (ex: Ollama proxy, LM Studio server, vLLM, etc.) via `backend/config/llm_config.example.json`.
